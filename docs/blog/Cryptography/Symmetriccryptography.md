...
| 时钟 | 状态 $(s_2, s_1, s_0)$ | 输出 ($s_0$) | 反馈 ($s_2 \oplus s_0$) | 下一状态 |
| :--: | :-------------------: | :----------: | :---------------------: | :-------: |
...
- **效率：** B-M 算法非常高效。对于一个由 $L$ 级 LFSR 生成的序列（线性复杂度为 $L$），只要已知密钥流片段长度**至少达到 $2L$ 个连续比特**，B-M 算法就能唯一地确定该 LFSR 的特征多项式（对 m 序列，长度 $\ge 2L$ 是成功重构的充要条件）。
...
3. **重构 LFSR：**
    如果密钥流确实是由一个 $L$ 级的 LFSR 生成的（例如一个 m序列），并且攻击者获得的密钥流片段长度至少为 $2L$ 比特（这是 Berlekamp-Massey 算法能唯一确定 LFSR 所需的最小长度），B-M 算法就能成功地计算出这个 LFSR 的特征多项式 $P(x)$ 和一个有效的初始状态。
...
一个 $L$ 级的 LFSR 由 $L$ 个反馈系数 (定义其特征多项式) 和 $L$ 个比特的初始状态所唯一确定。总共需要确定 $2L$ 个未知参数。因此，需要 $2L$ 个独立方程（即 $2L$ 个密钥流比特提供的约束）才能解出这些参数，且 $N\ge 2L$ 是 Berlekamp-Massey 算法唯一确定该 LFSR 的理论保证。B-M 算法系统地解决了这个问题。
...
流密码作为对称密码学的重要分支，在现代信息安全中发挥着重要作用。从简单的LFSR到复杂的非线性组合生成器，流密码的设计不断演进，以应对新的密码分析技术和安全威胁。

流密码的核心在于密钥流生成器的设计。一个优秀的密钥流生成器必须具备高安全性、长周期、良好统计特性和高效实现等特点。虽然单个LFSR由于其线性特性容易被Berlekamp-Massey算法攻破，但通过非线性组合、滤波或时钟控制等方法，可以构造出安全实用的流密码系统。

随着物联网、5G通信等新技术的发展，对低延迟、高吞吐量加密的需求日益增长，流密码凭借其速度快、错误传播小等优势，在这些领域具有重要的应用前景。同时，面对量子计算等新威胁，流密码的设计也需要不断创新和完善。
...
| 处理单位 | 比特/字节 | 固定长度块 |
| 加密速度 | 快 | 相对较慢 |
| 错误传播 | 小（通常1比特） | 取决于工作模式 |
| 内存需求 | 小 | 较大 |
| 实现复杂度 | 相对简单 | 较复杂 |
| 标准化程度 | 较低 | 高（如AES） |
| 主要应用 | 实时通信、无线网络 | 数据存储、网络协议 |

...
数论是密码学的基础学科之一，许多现代加密算法（如RSA、AES、ECC等）都依赖于数论中的基本概念和算法。系列作者[Wcowin](https://github.com/Wcowin)总结了密码学常用的数论知识（不分前后顺序）。

---

## 0.前言

在正式开始之前，我们先从小学知识出发，用三步走完整个数论。

**小学一年级：数苹果**

小明有 3 个苹果，小红又给了他 2 个。请问小明现在有几个苹果？

答案是 $3+2=5$。到这里，你已经掌握了**加法**——数论研究的第一类对象，到手了。

**小学三年级：分苹果**

有 7 个苹果，平均分给 3 个小朋友，每人分 2 个，还剩 1 个。写成算式就是：

$$
7 = 3 \times 2 + 1
$$

那个"还剩 1 个"，就是**余数**。恭喜你，你已经学会了**带余除法**——它在本文后面有一个更唬人的名字，叫**同余**。

**小学五年级：拆数字**

6 可以拆成 $1\times6$，也可以拆成 $2\times3$；而 7 只能写成 $1\times7$。

能拆开的叫**合数**，拆不开的叫**质数**（也叫素数）。到这里，你已经掌握了**质数与合数**——RSA 的核心原料，同样被你学会了。

**小结：**

至此，你**已经学会了**基本的数论。

没错，就是这么简单：数论的本质不过是把"分苹果"这件事问得再深一点——分不完怎么办？怎么分才算公平？怎么让别人分不出来你原来有几个苹果？

密码学做的事，也就是把这些问题换一身更抽象的衣服。下面我们把它翻译成密码学的语言，正式开始。  

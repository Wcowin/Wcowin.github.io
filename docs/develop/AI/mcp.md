...
- **IDE / 开发工具**：VS Code、Cursor、Zed、Continue、Windsurf 等
...
- **2025年3月**：OpenAI 全面支持 MCP
- **2026年初**：MCP 已明确成为 AI Agent 工具连接的事实标准
- **2026年5月**：GitHub 正式上线 MCP 服务市场，服务器数量从 30 个增长到 3000+ 个
- Google、Microsoft 等厂商快速跟进支持

### MCP 与 Skills 协议的关系

2026 年，Skills 协议正在挑战 MCP 的地位，两者关系：

| 维度 | MCP | Skills |
|------|-----|--------|
| **定位** | 工具连接协议 | 工作流定义语言 |
| **解决什么** | Agent 如何连接工具 | Agent 如何执行复杂任务流程 |
| **层级** | 基础设施层 | 应用编排层 |
| **关系** | 互补 | 互补 |

> 简单理解：MCP 提供「专业厨房」（工具访问），Skills 提供「菜谱」（工作流程）。两者结合使用效果更佳。
...
- 为不同服务器设置不同权限
- 定期审计服务器日志
- **防范 Prompt Injection**：MCP 服务器会将工具描述放入 Agent 上下文，恶意服务器可能通过工具描述进行 prompt 注入攻击，需谨慎选择可信来源的 MCP 服务器

...
## 五、OWASP Agentic Applications Top 10 for 2026

2025年底，OWASP GenAI 安全项目发布了专门针对 Agentic AI 的《Agentic Applications Top 10 for 2026》，这是经 100 多位专家同行评审的框架，专门针对自主规划、调用工具并执行决策的 AI 智能体：

| 排名 | 风险 | 说明 |
|------|------|------|
| **AA01** | **过度代理（Excessive Agency）** | Agent 拥有超出必要的自主权，可能导致危险操作 |
| **AA02** | **Prompt 注入与操纵** | 通过直接或间接注入控制 Agent 行为 |
| **AA03** | **工具链攻击** | 利用 Agent 调用的工具链进行攻击 |
| **AA04** | **记忆与上下文污染** | 篡改 Agent 的记忆或上下文影响其决策 |
| **AA05** | **Agent 间通信安全** | 多 Agent 系统中的通信安全问题 |
| **AA06** | **身份与授权绕过** | 伪造 Agent 身份或绕过授权机制 |
| **AA07** | **供应链与第三方风险** | MCP 服务器、Skill 等第三方组件的安全风险 |
| **AA08** | **观测性与审计缺失** | 缺乏对 Agent 行为的有效监控和审计 |
| **AA09** | **资源滥用与 DoS** | Agent 被利用进行资源耗尽攻击 |
| **AA10** | **人机协作边界模糊** | Human-in-the-loop 机制设计不当导致的风险 |

> **OpenAI Axios 恶意包事件**（2025年底）：攻击者通过伪装成官方 axios 包的恶意包，在开发者使用 AI 编码助手时植入后门。这一事件引发了业界对 Agentic AI 安全框架的集中讨论。
...
| 工具/资源 | 类型 | 说明 |
|-----------|------|------|
| [OWASP GenAI Security](https://genai.owasp.org/) | 标准 | OWASP AI 安全全球项目，含 LLM Top 10 和 Agentic AI Top 10 |
...
| **Agent 特有风险** | 过度授权、供应链攻击、工具调用链安全、MCP/Skill 第三方风险 |
| **协议安全** | MCP 用 OAuth + mTLS，A2A 用 AgentCard + HTTPS，遵循委托单调性原则 |
| **安全标准** | OWASP LLM Top 10（2025）+ Agentic Applications Top 10（2026）|
| **开发者必做** | 系统提示词加固、工具最小权限、不可逆操作人工确认、审计日志、供应链审计 |
